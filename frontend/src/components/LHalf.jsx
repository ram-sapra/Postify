...
export default function LHalf({ songName, artistName, duration }) {
  return (
    <div className="h-full col-span-7 flex flex-col overflow-hidden">
      ...
      <div className="max-h-[calc(100%-372px)] mt-6 w-[96%] overflow-auto">
        <div className="flex justify-between items-center h-[75px]  w-[99%] bg-[#74809b] rounded-3xl">
          <div className="h-[80%] ml-6 w-auto rounded-xl">
            <img
              className="h-full w-full object-cover rounded-xl"
              src="/images/albumCvr.jpeg"
              alt="Song album picture"
            />
          </div>
          <div className="w-[25%] font-bold flex justify-start items-center text-white text-3xl">
            {songName}
          </div>
          <div className="w-[30%] font-bold text-lg">{artistName}</div>
          <div className="font-semibold text-white text-lg">{duration}</div>
          <button className="font-semibold text-xl mr-10">
